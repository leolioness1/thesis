...
from data_generator_segmentation import DataGenerator_segmentation
import helper
train_generator_gt = DataGenerator_segmentation(train, dimension=(64, 64), size=64, norm_method='none',
                                               batch_size=len(train), n_channels=4)
train_sample_gt = train_generator_gt.__getitem__(0)
train_X=train_sample_gt[0]
train_Y=train_sample_gt[1]
...
def train_test_model(hparams, run_dir, name, n_epochs=5):
    ...
    train_generator = DataGenerator_segmentation(train, dimension=(height, width), size=height, norm_method=norm_name,
                                                 batch_size=batch_size,
                                                 n_channels=n_channels)
    val_generator = DataGenerator_segmentation(val, dimension=(height, width), size=height, norm_method=norm_name,
                                               batch_size=batch_size,
                                               n_channels=n_channels)
