...
\paragraph{}
Following this realisation, the size of the input batch was reduced to create a more even pixel-wise class distribution, the results of this experiment will be described in detail in the following section.
...
\subsection{Size of the input patch}
\paragraph{}
...
\begin{figure}[hbt!]
    ...
    \includegraphics[width=0.5\linewidth]{patch size_Validation dice coefficient.png}
...
\bottomrule
    \end{tabular}
  \end{center} 
  \caption{Dice coefficient and loss with varying patch size}\label{tab_patch}
\end{table}
...
\begin{figure}[hbt!]
    ...
    \caption{Normalisation method dice coefficient}
    \label{norm_dice}
    % \end{minipage}
    %     \hfill
    %     \begin{minipage}[c]{0.45\linewidth}
...
\begin{table}[ht!] 
    ...
    \end{tabular}
  \end{center} 
  \caption{Dice coefficient and loss with varying normalisation method}\label{tab_norm}
\end{table}
...
\begin{figure}[hbt!]
    % \begin{minipage}[c]{0.45\linewidth}
    \centering
    \includegraphics[width=0.5\textwidth]{augmentation method_Validation dice coefficient.png}
    \caption{Augmentation method validation dice coefficient comparison}
    \label{aug_dice}
    %     \end{minipage}
    %     \hfill
    %     \begin{minipage}[c]{0.45\linewidth}
...
\begin{figure}[hbt!]
    \begin{minipage}[c]{0.45\linewidth}
        ...
        \hfill
        \begin{minipage}[c]{0.45\linewidth}
        \includegraphics[width=\linewidth]{optimiser_Validation loss.png}
...
\begin{table}[ht!] 
    ...
    \end{tabular}
  \end{center} 
  \caption{Dice coefficient and loss with varying optimiser}\label{tab_opt}
\end{table}

\paragraph{}
Contradicting the literature, perhaps given the small dataset in this project compared with the millions of examples used to train state-of-the-art models, RMSProp outperforms the other optimisers when it comes to loss minimisation as seen in Figure \ref{opt_loss}.

\paragraph{}
Figure \ref{opt_dice} shows similar validation dice coefficient between RMSProp, \gls{NAdam} and \gls{Adam}, in order of maximisation performance, with \gls{SGD} lagging behind significantly, given the baseline learning rate of $0.0001$ and mini-batch size of $4$.
...
\bottomrule
    \end{tabular}
  \end{center} 
  \caption{Dice coefficient and loss with varying learning rate}\label{tab_lr}
\end{table}
\begin{figure}[hbt!]

\centering
    % \begin{minipage}[c]{0.45\linewidth}
    \includegraphics[width=0.5\linewidth]{learning rate_Validation dice coefficient.png}
...
In the grid search experiment, the initial learning rate of $0.01 $seemed to only perform well with the \gls{SGD} optimiser and vice versa, the \gls{SGD}  optimiser only performed well with the $0.01$ learning rate.
\paragraph{}
...
%\rowcolor{lightgray}
    ...
    \end{tabular}
  \end{center} 
  \caption{Dice coefficient and loss with varying batch size}\label{tab_batch}
\end{table}
...
%\rowcolor{lightgray}
    ...
    \end{tabular}
  \end{center} 
  \caption{Dice coefficient and loss with varying initialisation method}\label{tab_init}
\end{table}
...
\begin{figure}[hbt!]
    % \begin{minipage}[c]{0.45\linewidth}
    \centering
    \includegraphics[width=0.5\linewidth]{initilisation method_Validation dice coefficient.png}
...
\end{figure}    
\paragraph{}
From here on, uniform he initialisation will be used for the consequent experiments.
...
\begin{figure}[hbt!]
    % \begin{minipage}[c]{0.45\linewidth}
    \centering
    \includegraphics[width=0.5\textwidth]{activation functions_Validation loss.png}
...
\begin{table}[ht!]
    ...
    \rowcolor{lightgray} \gls{ELU} & 0.85 & 0.93 & 0.78 & 0.26 & 0.11 & 0.31  \\ \gls{ReLU} & 0.67 & 0.73 & 0.64 & 0.53 & 0.45 & 0.52  \\ tanh & 0.8 & 0.88 & 0.77 & 0.33 & 0.19 & 0.32  \\
                    \bottomrule
    \end{tabular}
    \caption{Activation function comparison of Dice Coefficient and Loss across datasets} \label{tab_act}
  \end{center}
\end{table}
...
The implementation of the loss functions in Python under the Tensorflow Keras framework required a substantial amount of effort, as they were custom losses and required special attention to make sure matrix dimensions were correct for backpropagation to work and training to be successful. Figure \ref{loss_loss} shows that they are all successfully training the model as the loss is decreasing in general as the number of epochs increase.


NEEED TO EXPAND ON WhY

\begin{figure}[hbt!]
    \begin{minipage}[c]{0.45\linewidth}
        \includegraphics[width=\linewidth]{loss function_Validation dice coefficient.png}
...
\begin{table}[ht!] 
    ...
    \end{tabular}
  \end{center} 
  \caption{Dice coefficient and loss with varying loss function}\label{tab_loss}
\end{table}

\paragraph{}
The best loss function is still the \gls{CE} dice loss so it will continue to be used in the following experiments.
...
\begin{figure}[hbt!]
    \begin{minipage}[c]{0.45\linewidth}
        ...
        \hfill
    \begin{minipage}[c]{0.45\linewidth}
        \includegraphics[width=\textwidth]{early stoppping_Validation loss.png}
        \caption{Loss function loss comparison of \gls{ES} usage}
        \label{es_loss}
    \end{minipage}
\end{figure}
...
\begin{figure}[hbt!]
    \begin{minipage}[c]{0.45\linewidth}
        ...
        \hfill
    \begin{minipage}[c]{0.45\linewidth}
        \includegraphics[width=\textwidth]{dropout_Validation loss.png}
        \caption{Loss function loss comparison of of removal of Dropout}
        \label{dropout_loss}
    \end{minipage}
\end{figure}
