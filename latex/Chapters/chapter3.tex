...
\subsubsection{Google Earth Engine (GEE) and JavaScript}
\paragraph{}
\gls{GEE} provides the ability of extracting satellite data using bespoke JavaScript queries through a Console \gls{UI} (shown in Figure \ref{gee_console_ui}), the Sentinel-2 data is available in the \gls{GEE} image collection with ID \textit{COPERNICUS/S2}.
    ...
    \begin{figure}[hbt!]
        ...
        \caption{GEE Console UI showing a snippet of the bespoke JavaScript script}
...
\subsection{Google Cloud Storage (GCS) and JPEG 2000 (JP2) data}
\paragraph{}
...
\begin{figure}[hbt!]
    \begin{minipage}[c]{0.45\linewidth}
        ...
        \caption{Example of the RGB channels normalised using z-score}
        \label{thaw_slump_img}        
        \end{minipage}
        \hfill
        \begin{minipage}[c]{0.45\linewidth}
        \includegraphics[width=\linewidth]{thaw_slump_mask_z_score.PNG}
        \caption{Example of a RTS mask}
...
\begin{table}[ht!] 
    ...
    \end{tabular}
    \end{center}
    \caption{Distribution of GEE data for scene classification}\label{table_label_data_class}
\end{table}
...
Figure \ref{rts_pixel_dist} depicts the distribution of pixels with \gls{RTS} positive label in the \textit{\gls{GEE} data}, as it can be seen from the figure most images have less than 500 pixels of the positive class which is less than $1\%$ of a $256$×$256$ pixel image.
    \begin{figure}[hbt!]
        ...
        \caption{Distribution of GEE data RTS positive pixels}
...
\begin{table}[ht!] 
    ...
    \hline
    \end{tabular}
    \caption{Train/ Validation/ Test split of samples for GEE data} \label{table_label_data_gee}
    \end{center}
\end{table}

Later in the project the \textit{\gls{JP2} data} was introduced with a total of $807$ $64$x$64$ slices containing positive \gls{RTS} pixels that can be used for the supervised \gls{DL} model its distribution is as follows:
\begin{table}[ht!] 
    \begin{center}
    \begin{tabular}{c|c|c}     
    \textbf{Dataset} & \textbf{Thaw Slump} & \textbf{Percentage of total} \\
    ...
    \end{tabular}
    \end{center}
    \caption{Train/Validation/Test split of samples for JP2 data} \label{table_label_data_jp2}
\end{table}
...
% as it can be seen from Figure \ref{thaw_slump_mask} in section \ref{rs_data_collection}.
    \begin{figure}[hbt!]
        ...
        \caption{Distribution of JP2 data RTS positive pixels across datasets}
...
The different architectures, as well as the architecture parameters (kernel sizes, stride, number of layers, number of filters among others), which nowadays are also considered hyperparameters that feed into the learning process, won't be described in section \ref{nn_hyperparameters} since these were already covered in section \ref{seg_nets} and \ref{cnn_layers}, respectively.
...
\end{enumerate}
\subsection{Deep Learning process} \label{nn_hyperparameters}
\paragraph{}
...
\paragraph{Binary Cross Entropy (CE)}
...
\paragraph{Interseciton over Union (IoU) loss}
...
\paragraph{Root Mean Square Propagation (RMSProp)}
...
\paragraph{Adaptive Momentum Estimation (Adam)}
...
\paragraph{Nestrov-accelerated Adaptive Momentum Estimation (NAdam)}
...
$X \sim \mathcal{U}(a,b)$
% X ~ Uni(a,b) where X draws its values from the uniform distribution
...
There is also a Xavier uniform variation, where the weights are chosen from a random uniform distribution, which will not be covered in detail.
...
\begin{figure}[hbt!]
    ...
    \caption{Convergence of a 30-layer CNN in He et al.'s paper (\cite{He_2015_ICCV})}
    \label{fig_he_xavier}
\end{figure}
...
In Figure \ref{fig_dropout} it can be seen that after dropout is applied in \ref{fig_dropout}(b) the network becomes less complex after being subsampled from the original \gls{NN} in \ref{fig_dropout}(a).
    ...
    \begin{figure}[hbt!]
        ...
        \caption{(a) Regular 2-layer NN (b) Example of NN after dropout applied. Crossed neurons have been turned off. (\cite{shanmugamani2018deep})}
        \label{fig_dropout}
    \end{figure}
...
\paragraph{}
(\cite{GoodBengCour16}) suggests that the best way of choosing an initial learning rate is to monitor learning curves, plots of loss function as a function of learning time, usually epochs. 
